...
static int __imx378_power_on(struct imx378 *imx378)
{
   ...
   */
	...
	ret = regulator_enable(imx378->supplies[i].consumer);
		if (ret) {
			dev_err(dev, "Failed to enable %s: %d\n",
				imx378->supplies[i].supply, ret);
...
disable_reg_clk:
	for (--i; i >= 0; i--)
		regulator_disable(imx378->supplies[i].consumer);		
...
+	 */
	for (i = IMX378_NUM_SUPPLIES - 1; i >= 0; i--) {
		ret = regulator_disable(imx378->supplies[i].consumer);
		if (ret)
